...
CREATE TABLE IF NOT EXISTS articles (
  ...
  slug VARCHAR(255) UNIQUE NOT NULL, -- 文章的唯一标识符（URL友好名称）
  ...
  tags TEXT[] DEFAULT '{}' -- 标签数组，用于存储文章标签
);

-- 创建评论表
CREATE TABLE IF NOT EXISTS comments (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY, -- 使用 UUID 作为主键
  article_id UUID NOT NULL REFERENCES articles(id) ON DELETE CASCADE, -- 关联的文章ID
  author_name VARCHAR(100) NOT NULL, -- 评论作者姓名
  author_email VARCHAR(255) NOT NULL, -- 评论作者邮箱
  content TEXT NOT NULL, -- 评论内容
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()      -- 创建时间
);

-- 创建索引以提高查询性能
CREATE INDEX IF NOT EXISTS idx_articles_published ON articles(published);       -- 根据发布状态索引
CREATE INDEX IF NOT EXISTS idx_articles_created_at ON articles(created_at DESC);    --  根据创建时间索引，降序排列
CREATE INDEX IF NOT EXISTS idx_articles_slug ON articles(slug);     -- 根据文章slug索引
CREATE INDEX IF NOT EXISTS idx_articles_tags ON articles USING GIN(tags); -- 使用 GIN 索引以支持数组类型的标签查询
CREATE INDEX IF NOT EXISTS idx_comments_article_id ON comments(article_id); -- 根据文章ID索引评论
CREATE INDEX IF NOT EXISTS idx_comments_created_at ON comments(created_at); -- 根据评论创建时间索引
